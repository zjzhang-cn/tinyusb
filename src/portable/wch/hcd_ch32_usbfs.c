 ...
 * written, with no frame-boundary alignment. Left to itself, an isochronous
 * endpoint that is re-submitted on completion (as the UAC audio class does)
 * is polled at the task-loop rate, far exceeding UAC 1.0's one-packet-per-
 * 1ms-frame contract. This driver paces isochronous transfers with the SOF
 * interrupt: each ISO endpoint is allowed at most one transaction per USB
 * frame, armed from the SOF tick and from the ISO completion handler. With one
 * IN and one OUT transaction per frame the capture/playback callbacks run at
 * ~1000 Hz instead of the runaway ~19000/3000 Hz.
 */
...
static bool hardware_start_xfer(uint8_t pid, uint8_t ep_addr, uint8_t data_toggle, bool is_iso) {
  ...
  // (e.g. 192-byte FS isochronous audio). Zero-length transfers (control
...
// while streaming. At most one endpoint is armed per call (single SIE).
...
void hcd_int_handler(uint8_t rhport, bool in_isr) {
  ...
  if (fg & USBFS_UIF_HST_SOF) {
    ...
    g_sof_frame++;
    // ISO has priority over the NAK retry: isochronous streaming must keep its
    // 1/frame cadence even while a control transfer is NAKing (the retry happens
    // in the gaps). Reversing this starves audio under a persistent control NAK.
    arm_iso_drain(true);
    arm_nak_retry();
    // fall through
  }
